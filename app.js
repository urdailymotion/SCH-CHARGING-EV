...
window.saveAppUsers = function(list) {
  ...
  if (typeof VoltFirebase !== 'undefined' && VoltFirebase.isConfigured() && Array.isArray(list)) {
    list.forEach(u => VoltFirebase.saveUser(u));
  }
};
...
function initTableControls() {
  ...
  if (formAdd) {
    formAdd.addEventListener('submit', (e) => {
      ...
      persistData();
      if (window.VoltFirebase && VoltFirebase.isConnected()) {
        VoltFirebase.addSwap(newRec);
      }
  ...
  if (formEdit) {
    formEdit.addEventListener('submit', (e) => {
      ...
      if (rec) {
        ...
        persistData();
        if (window.VoltFirebase && VoltFirebase.isConnected()) {
          VoltFirebase.updateSwap(id, rec);
        }
  ...
  if (btnDelete) {
    btnDelete.addEventListener('click', () => {
      if (deleteRecordTargetId) {
        ...
        persistData();
        if (window.VoltFirebase && VoltFirebase.isConnected()) {
          VoltFirebase.deleteSwap(idToDelete);
        }
...
function initProblemLogControls() {
  ...
  if (btnDoDelete) {
    btnDoDelete.addEventListener('click', () => {
      ...
      const deletedId = problemToDeleteId;
      ...
      persistData();
      if (typeof VoltFirebase !== 'undefined' && VoltFirebase.isConfigured()) {
        VoltFirebase.deleteProblem(deletedId);
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof VoltFirebase !== 'undefined') {
    VoltFirebase.init();
    if (typeof initFirebaseUIHandlers === 'function') {
      initFirebaseUIHandlers();
    }
  }
...
function handleDbEditSwapSubmit(e) {
  ...
  localStorage.setItem('voltswap_swaps', JSON.stringify(swapsData));
  if (window.VoltFirebase && VoltFirebase.isConnected()) {
    VoltFirebase.updateSwap(id, updated);
  }
...
window.confirmDeleteDbProblem = function(idOrIdx) {
  openDbConfirmModal(
    ...
    () => {
      ...
      localStorage.setItem('voltswap_problems', JSON.stringify(problemsData));
      if (typeof VoltFirebase !== 'undefined' && VoltFirebase.isConfigured()) {
        VoltFirebase.deleteProblem(idOrIdx);
      }
...
window.confirmDeleteDbSchedule = function(idx) {
  ...
  const targetItem = schedulesData[idx];
  openDbConfirmModal(
    ...
    () => {
      ...
      localStorage.setItem('voltswap_schedules', JSON.stringify(schedulesData));
      if (typeof VoltFirebase !== 'undefined' && VoltFirebase.isConfigured() && targetItem) {
        VoltFirebase.deleteSchedule(targetItem.id || targetItem);
      }
...
// =============================================================================
// 16. FIREBASE CLOUD FIRESTORE INTEGRATION & REALTIME SYNC HANDLERS
// =============================================================================
...
function openFirebaseConfigModal() {
  ...
  if (typeof VoltFirebase !== 'undefined') {
    const cfg = VoltFirebase.getSavedConfig();
    if (cfg) {
      const elKey = document.getElementById('fbApiKey');
      const elProj = document.getElementById('fbProjectId');
      const elAuth = document.getElementById('fbAuthDomain');
      const elBucket = document.getElementById('fbStorageBucket');
      const elSender = document.getElementById('fbMessagingSenderId');
      const elApp = document.getElementById('fbAppId');

      if (elKey) elKey.value = cfg.apiKey || '';
      if (elProj) elProj.value = cfg.projectId || '';
      if (elAuth) elAuth.value = cfg.authDomain || '';
      if (elBucket) elBucket.value = cfg.storageBucket || '';
      if (elSender) elSender.value = cfg.messagingSenderId || '';
      if (elApp) elApp.value = cfg.appId || '';
    }
  }
  openModal('modalFirebaseConfig');
}

function initFirebaseUIHandlers() {
  const form = document.getElementById('formFirebaseConfig');
  const btnParse = document.getElementById('btnParseFbConfig');
  const btnReset = document.getElementById('btnResetFbConfig');

  // Parse Raw Config pasted by user
  if (btnParse && !btnParse.dataset.bound) {
    btnParse.dataset.bound = 'true';
    btnParse.addEventListener('click', () => {
      const raw = (document.getElementById('fbConfigPasteArea')?.value || '').trim();
      if (!raw) {
        showToast('Tempel kode/JSON konfigurasi terlebih dahulu di kotak teks!', 'warning');
        return;
      }
      try {
        let parsed = null;
        ...
        if (parsed && (parsed.apiKey || parsed.projectId)) {
          ...
          showToast('✨ Konfigurasi berhasil diekstrak ke formulir!', 'success');
        } else {
          showToast('Tidak dapat menemukan apiKey atau projectId dalam teks tersebut.', 'warning');
        }
      } catch (err) {
        showToast('Format teks konfigurasi tidak valid: ' + err.message, 'danger');
      }
    });
  }
  ...
  if (form && !form.dataset.bound) {
    ...
    form.addEventListener('submit', (e) => {
      ...
      const cfg = { apiKey, projectId, authDomain, storageBucket, messagingSenderId, appId };
      if (typeof VoltFirebase !== 'undefined') {
        VoltFirebase.saveConfig(cfg);
        const ok = VoltFirebase.init();
        if (ok) {
          showToast('🔥 Berhasil terhubung ke Firebase Cloud Firestore!', 'success');
        } else {
          showToast('⚠️ Konfigurasi disimpan, sistem siap menggunakan Cloud Firestore.', 'info');
        }
        updateFirebaseBannerStatus();
      }
  ...
  if (btnReset && !btnReset.dataset.bound) {
    btnReset.dataset.bound = 'true';
    btnReset.addEventListener('click', () => {
      if (confirm('Putuskan koneksi Firebase Cloud Firestore dan kembali ke Mode Lokal?')) {
        if (typeof VoltFirebase !== 'undefined') {
          VoltFirebase.clearConfig();
        }
        const fields = ['fbApiKey', 'fbProjectId', 'fbAuthDomain', 'fbStorageBucket', 'fbMessagingSenderId', 'fbAppId', 'fbConfigPasteArea'];
        fields.forEach(f => {
          const el = document.getElementById(f);
          if (el) el.value = '';
        });
        closeModal('modalFirebaseConfig');
        showToast('🔌 Koneksi Firebase dinonaktifkan. Kembali ke mode lokal.', 'info');
        updateFirebaseBannerStatus();
      }
    });
  }
...
function updateFirebaseBannerStatus() {
  const isConn = (typeof VoltFirebase !== 'undefined' && VoltFirebase.isConfigured() && VoltFirebase.isConnected());
  ...
  const btnMigrate = document.getElementById('btnStartFirebaseMigration');
  ...
  if (isConn) {
    ...
    title.innerHTML = `🔥 Terhubung ke Cloud Firestore: <strong>${cfg ? cfg.projectId : 'Online'}</strong>`;
    if (sub) sub.textContent = 'Data transaksi dan jadwal tersinkronisasi otomatis secara realtime antar seluruh gawai/perangkat.';
    if (btnMigrate) {
      btnMigrate.innerHTML = '<span>🚀</span> Sinkronkan Ulang / Migrasikan Data ke Cloud';
    }
  } else {
    banner.style.background = '#fff7ed';
    banner.style.borderColor = '#fed7aa';
    title.style.color = '#9a3412';
    title.innerHTML = `🔥 Status Database: Mode Lokal (Offline Mode)`;
    if (sub) sub.textContent = 'Data tersimpan di perangkat ini. Klik tombol di kanan atas untuk menghubungkan ke Firebase Cloud Firestore.';
  }
}

async function triggerFirebaseMigration() {
  if (typeof VoltFirebase === 'undefined' || !VoltFirebase.isConfigured()) {
    showToast('Harap atur kredensial Firebase terlebih dahulu sebelum melakukan migrasi!', 'warning');
    openFirebaseConfigModal();
    return;
  }

  const pBox = document.getElementById('fbMigrationProgressBox');
  const pLabel = document.getElementById('fbMigrationLabel');
  const pPct = document.getElementById('fbMigrationPct');
  const pBar = document.getElementById('fbMigrationBar');
  const btn = document.getElementById('btnStartFirebaseMigration');

  if (pBox) pBox.style.display = 'block';
  if (btn) btn.disabled = true;

  try {
    const result = await VoltFirebase.migrateAllLocalData((pct, msg) => {
      if (pBar) pBar.style.width = pct + '%';
      if (pPct) pPct.textContent = pct + '%';
      if (pLabel) pLabel.textContent = msg;
    });

    showToast(`✅ Migrasi Sukses! ${result.totalItems} data berhasil diunggah ke Firestore Cloud.`, 'success');
    if (pLabel) pLabel.textContent = `✅ Selesai! ${result.totalItems} data tersimpan di Cloud Firestore.`;
    ...
    showToast('Gagal migrasi ke Firestore: ' + err.message, 'danger');
    if (pLabel) pLabel.textContent = '❌ Terjadi kesalahan: ' + err.message;
    if (btn) btn.disabled = false;
  }
}

// Realtime incoming updates handlers
window.handleFirestoreSwapsUpdate = function(remoteSwaps) {
  if (!Array.isArray(remoteSwaps) || remoteSwaps.length === 0) return;
  const map = new Map();
  (swapsData || []).forEach(s => { if (s && s.id) map.set(String(s.id), s); });
  remoteSwaps.forEach(s => { if (s && s.id) map.set(String(s.id), s); });

  swapsData = Array.from(map.values());
  localStorage.setItem('voltswap_swaps', JSON.stringify(swapsData));
  ...
  if (typeof renderHistoryDaily === 'function') renderHistoryDaily();
  if (typeof updatePpaExecutiveDashboard === 'function') updatePpaExecutiveDashboard();
  if (typeof renderDbSwapsTable === 'function') renderDbSwapsTable();
  if (typeof updateDbSummaryMetrics === 'function') updateDbSummaryMetrics();
};

window.handleFirestoreProblemsUpdate = function(remoteProblems) {
  if (!Array.isArray(remoteProblems) || remoteProblems.length === 0) return;
  const map = new Map();
  (problemsData || []).forEach(p => { if (p && p.id) map.set(String(p.id), p); });
  remoteProblems.forEach(p => { if (p && p.id) map.set(String(p.id), p); });
  problemsData = Array.from(map.values());
  localStorage.setItem('voltswap_problems', JSON.stringify(problemsData));

  if (typeof renderProblemTable === 'function') renderProblemTable();
  if (typeof renderDbProblemsTable === 'function') renderDbProblemsTable();
  if (typeof updateDbSummaryMetrics === 'function') updateDbSummaryMetrics();
};

window.handleFirestoreSchedulesUpdate = function(remoteSchedules) {
  if (!Array.isArray(remoteSchedules) || remoteSchedules.length === 0) return;
  schedulesData = remoteSchedules;
  localStorage.setItem('voltswap_schedules', JSON.stringify(schedulesData));

  if (typeof renderScheduleTable === 'function') renderScheduleTable();
  if (typeof renderDbSchedulesTable === 'function') renderDbSchedulesTable();
  if (typeof updateDbSummaryMetrics === 'function') updateDbSummaryMetrics();
};

window.handleFirestoreUsersUpdate = function(remoteUsers) {
